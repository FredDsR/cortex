...
#
# SUMMARY_CONFLICT env var selects the SUMMARY.md conflict policy:
#   resolve (default) - take upstream, then signal "regenerate-needed". Right for a
#                       session-start pull, where local SUMMARY.md is stale.
#   surface           - treat a SUMMARY.md conflict like a task-file conflict (abort,
#                       exit 2). Right for the commit path, where the just-committed
#                       SUMMARY.md is the writer's fresh edit and must not be clobbered.
set -euo pipefail

WORK_DIR="${HOME}/.work"
SCRIPTS_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SUMMARY_MODE="${SUMMARY_CONFLICT:-resolve}"
...
# task/other conflicts always surface. SUMMARY.md conflicts surface only in
# 'surface' mode (the commit path); in 'resolve' mode they auto-resolve below.
must_surface="$task_conflicts$other_conflicts"
[[ "$SUMMARY_MODE" == "surface" ]] && must_surface="$must_surface$summary_conflicts"

if [[ -n "$must_surface" ]]; then
    # Surface and abort.
    git rebase --abort >/dev/null 2>&1 || true
