...
The workspace slug identifies a logical project — a git repo, a directory, or a manually registered area. All `~/.cortex/workspaces/<slug>/` paths depend on this resolution.
...
2. **`.meta` registry match.** Scan `~/.cortex/workspaces/*/.meta`; if any entry's `cwd:` field equals the current absolute CWD, use that workspace's directory name.
