...
- `cortex-kb`: authors `knowledge/<slug>.md` and `workbench/<slug>.md` via the `cortex kb` CLI, derives `index.md` (OKF section 8) and `log.md` (section 9) from the store, and exchanges OKF bundles with other tools via `cortex okf export` / `cortex okf import`.
...
```bash
bash "$HOME/.claude/skills/cortex-tracking/scripts/session_start.sh"
```
...
6. **Injection available.** If a workspace resolves and `cortex inject status` reports the hook is not wired, you MAY offer to enable opt-in session-start injection (`cortex-inject`). Never auto-enable it.

## Checkpoint Rules

| Trigger | Action |
|---------|--------|
| Session start | Run `session_start.sh` (above). If the session has a `github:` frontmatter field, invoke `cortex-github` for drift detection. If `SYNC` is `regenerate-needed` or `conflict`, follow the protocol in `cortex-sync/SKILL.md`. |
| After a `git commit` on a tracked branch | Append commit subject to the active task's **Scope** or **Notes**. |
| User asks "overview" / "status" / "where are we" | Run `bash $SKILL_DIR/scripts/manifest.sh` for a one-row-per-task TSV snapshot; only read individual task files for In-Progress / Blocked items. Regenerate SUMMARY.md from tasks/ + `git log` (if a repo) only when the user asks for a full rewrite. If GitHub is configured, also invoke `cortex-github`. |
...
If a session's SUMMARY.md has a `github: <owner>/<repo>` frontmatter field, invoke `cortex-github` for PR sync. Otherwise skip entirely — `gh` is not needed for non-code workspaces.
...
- Use hooks or background automation, with ONE opt-in exception: session-start context injection, off by default, via `cortex-inject` (`cortex inject`).
...
- `../cortex-kb/scripts/migrate_kb_frontmatter.py`: one-shot migrator for existing knowledge/workbench docs (backfills `updated`, canonical field order, regenerates the derived index banner, reports stale `work-kb`/`work-viz` references). Dry-run by default; `--write` to apply. The family's unified CLI is `cortex` (`cortex kb ...`, `cortex viz ...`, `cortex query ...`).
