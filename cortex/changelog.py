...
from cortex.kb.common import (home_dir, md_escape, more_notice, parse_max,
                              sync_after)
from cortex.sync.repo import git
...
# `SLUG`), because a store also holds hand-written subjects that merely
...
def read_history(kdir: Path, *, since: str = "") -> History:
    ...
    args += ["--", "."]
    r = git(args, cwd=kdir, capture=True)
    if r.returncode != 0:
    ...
    st = git(["status", "--porcelain", "-uall", "--", "."], cwd=kdir, capture=True)
...
def render(kdir: Path, hist: History, *, max_n: int | None, banner: bool) -> list[str]:
    ...
    for e in shown:
        ...
        body = (f"[{md_escape(text)}]({target})" if target else md_escape(text))
        lines.append(f"* **{e.label}**: {body} - {desc}")
    more = more_notice(len(hist.entries), max_n)
    if more:
        lines += [""] + more
    return lines
